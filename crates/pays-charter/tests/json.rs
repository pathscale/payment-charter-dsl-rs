//! A minimal JSON reader for the conformance harnesses.
//!
//! `pays-charter` may take dependencies, but a test-only reader for a format this constrained
//! is less to justify than one, and it keeps the suite runnable with no network. Shared by the
//! eval-vector and authenticity harnesses so there is one copy rather than two that drift.
